...
#define USER_WP_CLEAR (USER_WP_US_PERM_WP_DIS | USER_WP_US_PWR_WP_DIS | USER_WP_US_PERM_WP_EN | USER_WP_US_PWR_WP_EN)
...
int do_read_extcsd(int nargs, char **argv)
{
	...
	if (ext_csd_rev >= 5)
	{
		...
		printf("Enhanced User Data Start Address"
			   ...
			   regl);
		printf(" i.e. %llu bytes offset\n", (is_blockaddresed(ext_csd) ? 512ll : 1ll) * regl);
...
do_retry:
	...
	while (bytes_left)
	{
		unsigned int chunk_size = bytes_left < default_chunk ? bytes_left : default_chunk;
...
{
	if (block_data_buff[0] == 84 && block_data_buff[1] == 114 && block_data_buff[2] == 97 && block_data_buff[3] == 110 && block_data_buff[4] == 115 && block_data_buff[5] == 99 && block_data_buff[6] == 101 && block_data_buff[7] == 110 && block_data_buff[8] == 100) // TRANSCEND
	{
...
int do_sd_erase(int nargs, char **argv)
{
	...
	strcpy(pwd, "transcend");
	pwd_len = strlen(pwd);
	cmd42_para = MMC_CMD42_SET_LOCK; // do lock first
	block_size = MMC_BLOCK_SIZE;
	ret = set_block_len(fd, block_size);
	...
	block_size = 2;						 // set blk size to 2-byte for Force	Erase @DDR50 compability else block_size = MMC_BLOCK_SIZE;
